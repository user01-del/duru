...
<jsp:useBean id="Member" calss="Member.Member" scope="page" />
...
<body>
	<%
		...
		if(result == 1){
			...
			PrintWriter script = response.getWriter();
			script.println("<script>");
			script.println("alert('비밀번호가 틀립니다')"); //로그인 성공 시 메인으로 이동
			script.println("history.back()");
			script.println("</script>");
		}else if(result == -1){
			PrintWriter script = response.getWriter();
			script.println("<script>");
			script.println("alert('존재하지 않는 아이디 입니다')"); //로그인 성공 시 메인으로 이동
			script.println("history.back()");
			script.println("</script>");
		}
	%>
